...
async function getSongs(category: string): Promise<SongDoc[]> {
  ...
  const queryMap: Record<string, object> = {
    "2026":   { movieYear: 2026, published: true },
    latest:   { releaseDate: { $gte: thirtyDaysAgo }, published: true },
    trending: { published: true },
    classics: { movieYear: { $lte: 2010 }, published: true },
    singers:  { published: true },
  };
  ...
  const sortMap: Record<string, object> = {
    ...
    trending: { views: -1 },
    classics: { views: -1 },
    singers:  { singerName: 1 },
    default:  { releaseDate: -1 },
  };

  const filter = queryMap[category] ?? { published: true };
  const sort   = sortMap[category] ?? sortMap["default"];

  const docs = await Song.find(filter)
    .select("title slug movieSlug movieTitle movieYear singerName singerSlug thumbnailUrl views releaseDate")
    .sort(sort)
    .limit(48)
    .lean();
...
function JsonLd({ songs, category, cfg }: { songs: SongDoc[]; category: string; cfg: (typeof CATEGORY_CONFIG)[string] }) {
  ...
  const itemList = {
    ...
    itemListElement: songs.slice(0, 10).map((s, i) => ({ "@type": "ListItem", position: i + 1, url: `${base}/songs/${s.movieSlug}/${s.slug}`, name: s.title })),
  };
  const breadcrumb = {
...
function SongCard({ song }: { song: SongDoc }) {
  const views =
    song.views >= 1_000_000 ? `${(song.views / 1_000_000).toFixed(1)}M views`
    : song.views >= 1_000   ? `${(song.views / 1_000).toFixed(0)}K views`
    : song.views > 0        ? `${song.views} views`
    : "";

  return (
    <Link href={`/songs/${song.movieSlug}/${song.slug}`}
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-gray-500 text-xs mt-0.5 truncate">
          {song.singerName}{song.singerName && song.movieTitle ? " · " : ""}{song.movieTitle}
        </p>
        {views && <p className="text-gray-600 text-xs mt-0.5">{views}</p>}
      </div>
